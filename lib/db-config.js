...
export function getMariaDbConfig(databaseUrl) {
  ...
  const config = {
    ...
    // Default pool size (10) queues requests once a page fires more concurrent
    // queries than that — the admin dashboard alone fires ~24. Raised to reduce
    // queueing; stays well under Aiven's free-tier connection cap.
    connectionLimit: 20,
  }
